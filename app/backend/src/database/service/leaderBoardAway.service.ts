...
const name = (matches: any, teams: any) => {
  const result = matches.map((match : any) => {
    const reduceteams = teams.reduce((acc: any | undefined, currentValue: any) => {
      if (currentValue.id === match.awayTeamId) {
...
const seVitoria = (test: any, match: any) => {
  const obj = test;
  obj.totalVictories += 1; obj.totalPoints += 3; obj.goalsFavor += match.awayTeamGoals;
  obj.goalsOwn += match.homeTeamGoals;
};

const seEmpate = (test: any, match: any) => {
  const obj = test;
  obj.totalDraws += 1; obj.totalPoints += 1; obj.goalsFavor += match.awayTeamGoals;
  obj.goalsOwn += match.homeTeamGoals;
};
const registerData = (matches: any, team: any, obj: objetoLeaderBoard) => {
  const test = obj;
  matches.forEach((match: any) => {
    if (team.id === match.awayTeamId && !match.inProgress) {
      test.totalGames += 1;
      if (match.awayTeamGoals > match.homeTeamGoals) {
        ...
        test.totalLosses += 1; test.goalsFavor += match.awayTeamGoals;
        test.goalsOwn += match.homeTeamGoals;
      }
...
const leaderBoardAwayTeam = async () => {
...
export default leaderBoardAwayTeam;
